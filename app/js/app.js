...
(function () {
    ...
    window.exportAdminGradeTable = function (kind, format) {
        ...
        if (format === 'csv') {
            downloadTextFile(baseName + '.csv', 'text/csv;charset=utf-8', rowsToCsv(headers, rows));
            return;
        }

        if (format === 'csv-split') {
            groupedExportSheets(kind, rows).forEach(function (sheet, idx) {
                setTimeout(function () {
                    var filename = 'admin-' + kind + '-' + safeFilenamePart(sheet.name) + '-' + datePart + '.csv';
                    downloadTextFile(filename, 'text/csv;charset=utf-8', rowsToCsv(headers, sheet.rows));
                }, idx * 250);
            });
            return;
        }
    ...
    function renderExam() {
        ...
        var briefBody = '<div style="' + ccss + '">'
            ...
            + '<b>2 hours · Labs 5–10 · 5 parts (A–E) · individual · submit by <code>git push</code>.</b><br>'
    ...
    function renderFileView(filePath) {
        ...
        if (ext === 'md') {
            ...
            html = '<div style="padding:6px 12px;background:rgba(0,255,65,0.04);border-bottom:1px solid var(--border);font-size:11px;flex-shrink:0;display:flex;align-items:center;gap:6px;">'
                ...
                + '<button class="md-toc-btn" id="md-toc-toggle" title="Table of contents">&#9776; TOC</button>'
                + '</div>'
                + '<div style="position:relative;flex:1;display:flex;overflow:hidden;">'
                + '<div class="md-viewer" id="md-render" style="max-height:none;flex:1;overflow-y:auto;"></div>'
                + '<div class="md-toc-panel" id="md-toc-panel"></div>'
                + '</div>';
